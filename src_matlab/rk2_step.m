...
%   convective/diffusive RHS terms are. We therefore:
%     1. Evaluate the momentum RHS (no pressure) at the current state -> k1
%     2. Take an Euler sub-step to an intermediate state
...
%        divergence-free
%   This is the standard simplification for explicit RK2 projection methods:
%   continuity only needs to hold for the final combined step, so a single
%   end-of-step projection suffices. Temperature is advanced the same way
%   (Heun), using the new divergence-free velocity for the second stage's
%   convective transport.
    ...
    w_tilde = w;
    w_tilde(:, 2:n_z) = w(:, 2:n_z) + dt * k1_w;
    u_tilde = u;
    u_tilde(2:end-1, :) = u(2:end-1, :) + dt * k1_u;
    w_tilde = apply_bc_w(w_tilde);
    u_tilde = apply_bc_u(u_tilde);
