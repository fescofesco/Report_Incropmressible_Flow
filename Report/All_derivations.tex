...
scales do not cancel. See Appendix~B of the main report (\texttt{main.pdf}) for the full
re-derivation.
...
A second-order explicit Adams--Bashforth scheme is employed. Forward Euler is
used for the first step, after which the previous RHS is retained.
...
\begin{enumerate}
    \item \textbf{Predictor step}: Apply AB2 to the convective and diffusive
    momentum RHS and include the known gradient $-\nabla p^n$ to obtain
    intermediate velocities $u^{**}, w^{**}$.
...
See Appendix~B of the main report (\texttt{main.pdf}) for the full derivation and literature sources.
...
Appendix~B of the main report (\texttt{main.pdf}).
...
\input{derivation_cylindrical_coordinates}

\input{1_2D_cartesian_xy}

\input{2_2D_cylindrical_coordinates_uw}

\input{3_2D_cylindrical_coordinates_uv}
