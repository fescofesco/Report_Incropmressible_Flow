...
\coordinate (A) at (\phia:\rin);
\coordinate (B) at (\phib:\rin);
\coordinate (D) at (\phia:\rout);
\coordinate (C) at (\phib:\rout);

% ---- filled control volume: a true annular sector (r,phi is the physical plane) ----
\fill[green!22] (A) arc[start angle=\phia, end angle=\phib, radius=\rin]
                 -- (C) arc[start angle=\phib, end angle=\phia, radius=\rout]
                 -- cycle;
\draw[black, very thick]
     (A) arc[start angle=\phia, end angle=\phib, radius=\rin]
     -- (C) arc[start angle=\phib, end angle=\phia, radius=\rout]
     -- cycle;

% ---- scalar node p at cell centre ----
\coordinate (P) at (\phim:\rm);
\filldraw[blue] (P) circle (2.2pt);
\node[blue, font=\small, below=4pt] at (P) {$p_{i,j}$};

% ---- u (radial velocity) on the r-faces, red ----
\draw[red,thick,->] (\phim:\rin-0.32) -- (\phim:\rin+0.24);
\node[red,font=\small, anchor=north] at (\phim:\rin-0.32) {$u_{i-\frac12,j}$};
\draw[red,thick,->] (\phim:\rout-0.24) -- (\phim:\rout+0.32);
\node[red,font=\small, anchor=south] at (\phim:\rout+0.32) {$u_{i+\frac12,j}$};

% ---- v (azimuthal velocity) on the phi-faces, teal ----
\draw[teal,thick,->] (\phia+7:\rm) -- (\phia-7:\rm);
\node[teal,font=\small, anchor=north east] at (\phia-9:\rm) {$v_{i,j-\frac12}$};
\draw[teal,thick,->] (\phib-7:\rm) -- (\phib+7:\rm);
\node[teal,font=\small, anchor=south west] at (\phib+9:\rm) {$v_{i,j+\frac12}$};

% ---- r_i, r_{i+1} leader labels, tucked near the origin ----
\draw[gray!70] (\phia-22:1.15) -- (A);
\node[font=\small] at (\phia-30:1.0) {$r_i$};
\draw[gray!70] (\phia-22:2.05) -- (D);
\node[font=\small] at (\phia-30:2.2) {$r_{i+1}$};

% ---- phi_{j-1}, phi_j labels near the origin, next to the Delta-phi arc ----
\node[font=\small] at (\phia+3:1.55) {$\phi_{j-1}$};
\node[font=\small] at (\phib-3:1.65) {$\phi_j$};

% ---- Delta r dimension, on the far (outer) side ----
\draw[<->] (\phib+15:\rin) -- (\phib+15:\rout)
    node[midway, sloped, above, font=\small] {$\Delta r$};

% ---- Delta phi angle indicator near the origin ----
\draw[purple!70,densely dashed] (0,0) -- (\phia:1.0);
\draw[purple!70,densely dashed] (0,0) -- (\phib:1.0);
\draw[purple!70,->] (\phia:0.55) arc[start angle=\phia, end angle=\phib, radius=0.55];
\node[purple!70,font=\small] at (\phim:0.8) {$\Delta\phi$};
\filldraw[black] (0,0) circle(0.8pt);
\node[font=\scriptsize, below=1pt] at (0,0) {$O$};

% ---- legend ----
\begin{scope}[shift={(4.5,1.6)}]
    \filldraw[blue] (0,0.85) circle (2.2pt);
    \node[font=\small,right] at (0.15,0.85) {$p$ (cell centre)};
    \draw[red,thick,->] (0,0.45) -- (0,0.7);
    \node[font=\small,right] at (0.15,0.58) {$u$ ($r$-face)};
    \draw[teal,thick,->] (0,0.15) -- (0.25,0.15);
    \node[font=\small,right] at (0.35,0.15) {$v$ ($\phi$-face)};
\end{scope}

\end{tikzpicture}
\caption{Staggered grid in the $(r,\phi)$ plane. Scalars ($p$) at the cell centre (blue);
         radial velocity $u$ on the $r$-faces (red); azimuthal velocity $v$ on the
         $\phi$-faces (teal). Unlike the $(r,z)$ grid used for the axisymmetric
         pipe-flow problem, here $(r,\phi)$ \emph{is} the physical plane itself, so the
         control volume is drawn
         as a true annular sector of radial extent $\Delta r$ and angular extent
         $\Delta\phi$ rather than a swept projection; its area element is
         $dA = r\,dr\,d\phi$, so the outer face at $r_{i+1}$ is physically longer than
         the inner face at $r_i$.}
...
\begin{tikzpicture}[scale=1.0]
    ...
    % Axes -- kept in a clear corner, away from the ghost-band labels
    \draw[->] (-0.9,-0.9) -- (-0.1,-0.9) node[right,font=\small]{$r$};
    \draw[->] (-0.9,-0.9) -- (-0.9,-0.1) node[above,font=\small]{$\phi$};
