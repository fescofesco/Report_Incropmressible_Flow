...
def compute_rhs_T(T, u, w, r_c, r_f, dr, dz, Re, Pr):
    ...
    so that advance_temperature() / an RK2 stage can each apply their own
    time-stepping weight to it (needed for the 2nd-order Heun scheme in
    time_integration.py).
...
def advance_temperature(T, u, w, r_c, r_f, dr, dz, dt, Re, Pr):
    ...
    compute_rhs_T directly to build the 2nd-order Heun (RK2) update.
